...
\title{Deuxième compte rendu\\-- Transporteur de valises --}
...
Le transporteur de valise doit accomplir trois tâches principales en regard du programme : suivre l'utilisateur, communiquer avec lui et éviter les obstacles.
...
L'idée d'utiliser une caméra vient de la difficulté d'utiliser les techniques étudiées ci-dessus, et le matériel étant déjà prévu puisque la détection d'obstacle se fait avec des caméras. L'idée ici est d'utiliser une approche par couleur : l'utilisateur doit porter un T-shirt particulier et doit être dans le champ de vision du robot, ce qui est très contraignant, mais c'était le plus simple à mettre en œuvre, et ça à l'avantage d'être fonctionnel.
% TODO détail du principe et des expérimentations
...
\paragraph{Calibration des caméras.}
...
\paragraph{Calcul de la carte de disparité.}
...
\begin{figure}
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    \caption{Résultats obtenus avec la stéréovision.}
    \label{disp_result}
\end{figure}

\subsection{Approche ABOD.}
\paragraph{Principe} L'idée de cet algorithme est d'identifier l'apparence du sol sur le flux vidéo d'une caméra et à partir de là de différentier le sol des obstacles en se basant sur leur apparence (un exemple de résultat peut être observé sur la figure \ref{abod}). Cette algorithme possède quelques limites cependant : un obstacle comme un bord de table apparaissant dans un coin sera repéré comme un objet éloigné. En général, un obstacle \emph{volant} sera considéré comme au sol mais éloigné.
...
\paragraph{Algorithme} L'idée de l'algorithme est assez simple : le sol est identifié par un histogramme, et chaque pixel de l'image est comparé à l'histogramme selon certains critères et sera classé comme appartenant au sol ou comme étant un obstacle. Afin d'affiner la détection, on décide de travailler dans l'espace de couleur \emph{HSV} et d'utiliser un procédé d'apprentissage. Lors de l'apprentissage, plusieures images du sol seront analysées par un programme qui combinera leurs histogrammes et enregistrera l'histogramme résultant dans un fichier xml. Ce fichier sera chargé par le robot qui l'utilisera pour différencier le sol des obstacles. Il ne peut donc fonctionner que sur un seul type de sol, ce qui n'est pas particulièrement génant, puisqu'il est sensé fonctionner dans un seul lieu.

% TODO expérience et résultats.

\section{Conclusion.}
Du aux limites de nos capacités techniques, mathématiques et temporelles, de nombreux compromis ont du être fait. Cependant, il reste envisageable d'avoir un prototype fonctionnel bien que limité d'ici la fin de l'année.
